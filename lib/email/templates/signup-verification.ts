interface RenderSignupVerificationEmailParams {
  code: string;
}
...
export function renderSignupVerificationEmail({
  code,
}: RenderSignupVerificationEmailParams): RenderedEmail {
  const subject = "[소독노트] 이메일 인증번호";
  const heading = "이메일 인증 요청";
  const description = "이메일 인증을 위해 아래 인증번호를 입력해 주세요.";
